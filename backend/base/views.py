# function based views are gonna be created
...
def get_user_profile_data(request,pk):
    try:
        try:
            ...
            return Response({'error':f'user {pk} not exist'},status=status.HTTP_404_NOT_FOUND)
    ...
    # user who would be logged in will be is_our_username=true holder and others as false
    except:
        return Response({'error':f'error getting user {pk}'},status=status.HTTP_400_BAD_REQUEST)
    
# In Django REST Framework (DRF), the many parameter in a serializer tells DRF whether you're serializing a single object or a queryset (multiple objects).

# many=True → Use when serializing a list of objects (querysets).
# many=False (or default behavior) → Use when serializing a single object (like one user).
...
def create_posts(request):
    try:
        ...
        serailizer =PostSerializer(post,many=False)
